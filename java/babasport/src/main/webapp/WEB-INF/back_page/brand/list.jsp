...
function optDelete(name,isDisplay){
	...
	$("#jvForm").attr("method","post").attr("action","/brand/deletes.do?name="+name+"&isDisplay="+isDisplay).submit();
}
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${pagination.list }" var="brand">
			<tr>
				...
				<td align="center">
				<a class="pn-opt" href="toEdit.do?id=${brand.id }">修改</a> | <a class="pn-opt" 
					onclick="if(!confirm('您确定删除吗？')) {return false;} 
					window.location.href='/brand/delete.do?id=${brand.id}&name=${name }&isDisplay=${isDisplay }'" href="javascript:void(0)">删除</a>
				</td>
			</tr>
		</c:forEach>		
	</tbody>
